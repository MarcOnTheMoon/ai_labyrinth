...
class RewardsByAreas(Rewards):
    ...
    def get_progress(self):
        """
        get the area where the ball is located.
        Needed for evaluation switching between neuronal nets HOLES_8 and HOLES21
    ...
    def __areas(self, layout):
        ...
        areas = {
            ...
            Layout.HOLES_21: [
                [-2.51, 2.59, -11.4, -8.82],   [2.59, 9.03, -11.4, -8.82],    [5.19, 9.18, -8.82, -5.28],  [5.29, 7.93, -5.28, -0.73],  [2.69, 5.29, -0.73, 2.07],
                [5.29, 7.73, -0.19, 4.55],     [7.73, 9.27, 1.08, 5.35],      [9.27, 13.7, 3.91, 7.47],    [9.27, 13.7, 7.47, 9.49],    [4.47, 9.27, 7.07, 9.49],
                [2.56, 7.18, 4.85, 7.07],      [0.52, 2.56, 5.73, 7.07],      [-2.2, 0.52, 5.73, 9.08],    [-4.8, -2.2, 6.77, 9.08],    [-4.8, -2.2, 3.22, 6.77],
                [-5.24, -2.2, 0.61, 3.22],     [-3.7, -2.2, -1.88, 2.56],     [-2.2, 0.21, -1.88, 2.56],   [-0.71, 2.49, -4.29, -1.88], [-0.94, 2.49, -8.32, -4.29],
                [-4.4, -0.94, -8.32, -5.4],    [-2.51, -0.94, -5.4, -3.77],   [-4.4, -0.94, -3.77, -2.35], [-6.47, -4.4, 7.04, -2.35],  [-9.56, -6.74, -7.04, -4.33],
                [-11.93, -9.56, -8.28, -5.88], [-13.7, -11.39, -8.28, -3.77], [-13.7, -9.54, -3.77, 0.0],  [-9.54, -5.77, -1.84, 0.26], [-7.41, -5.77, 0.26, 2.99],
                [-10.0, -6.47, 2.99, 5.78],    [-9.48, -7.18, 5.78, 9.73],    [-9.14, -3.64, 9.73, 11.4],  [2.64, 3.87, 9.73, 11.4]]
        }
        return areas[layout]
    ...
    def __target_points(self, layout):
        ...
        target_points = {
            ...
            Layout.HOLES_21: [
                [-2.91, -10.23], [-2.91, -10.23], [8.03, -9.03],   [6.71, -5.77],   [5.94, -2.34],
                [4.58, -0.02],   [7.09, 1.47],    [8.77, 3.87],    [12.68, 6.96],   [9.77, 8.8],
                [5.74, 7.73],    [3.35, 6.12],    [1.2, 5.93],     [-1.42, 7.6],    [-2.83, 7.42],
                [-2.99, 3.82],   [-2.83, 7.42],   [2.99, 3.82],    [-0.31, -1.35],  [-0.21, -3.67],
                [-0.16, -7.33],  [-1.57, -6.2],   [-1.47, -4.37],  [-3.59, -2.53],  [-6.13, -5.56],
                [-8.62, -6.05],  [-10.6, -7.25],  [-12.73, -4.33], [-10.22, -0.96], [-6.53, -0.38],
                [-6.37, 2.37],   [-8.43, 5.14],   [-8.23, 9],      [-3.19, 10.5]]
        }
    ...
    def __init_reward_values(self, layout):
        ...
        match layout:
            ...
            case Layout.HOLES_21:
                self.__destination = 1500
                self.__in_hole = -300
                self.__near_hole = -15
                self.__correct_direction = -1
                self.__interim = lambda progress, areas: 9 / len(areas) * (len(areas) - progress)
                self.__default = -2
