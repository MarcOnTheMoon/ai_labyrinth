...
class LabyrinthAgentDQN:
    ...
    def train(self, batch_size):
        ...
        for state, action, reward, next_state, done in minibatch:
            ...
            # Vorhersage der Q-Werte für den aktuellen Zustand (state)
            target_f = self.model.predict(state)  #target_forecast = Zielvorhersage
            target_f = target_f.reshape((9, 9))

            if done:
                target = reward # Wenn die Episode abgeschlossen ist (done), wird das Ziel (target) auf die Belohnung (reward) gesetzt. da es keinen nachfolgenden Zustand gibt hinter dem erreichten
            if not done:
                # Vorhersage der Q-Werte für den nächsten Zustand
                next_q_values = self.model.predict(next_state)
                next_q_values = next_q_values.reshape((9, 9)) # in (x,y) konvertieren
                # Berechnung des Zielwerts mit der Q-Learning-Formel
                target = reward + self.gamma * np.amax(next_q_values) #berechnet das Ziel (target) basierend auf dem Q-Learning-Update-Regel (Erkundung)
    ...
    def training(self, env):
        ...
        for episode in range(episodes):
            ...
            """if len(self.memory) >= batch_size:  # Überprüfen, ob genügend Erfahrungen im Speicher sind
                self.train(batch_size)"""  # Der Agent führt das Training basierend auf den gespeicherten Erfahrungen durch
            batch_size = int(batch_size / 5)
            if batch_size > 0 and not truncated: #nur lernen wenn entweder ziel erreicht oder ins Loch gefallen? sinnvoll?
