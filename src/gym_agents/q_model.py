...
class PtQNet(nn.Module): # PtQNet, die von nn.Module erbt. nn.Module ist die Basisklasse für alle neuronalen Netzwerke in PyTorch
    ...
    def __init__(self, state_size, action_size, fc1_units = 2048, fc2_units = 1024, fc3_units = 128):
        """
    ...
    def forward(self, state):
        ...
        """
        x = F.leaky_relu(self.bn1(self.fc1(state))) #Der Eingabezustand (state) wird durch die erste vollverbundene Schicht fc1 geleitet, dann durch die Batch Normalisierung und dann durch die leaky ReLU-Aktivierungsfunktion. Das Ergebnis wird in x gespeichert.
        x = F.leaky_relu(self.bn2(self.fc2(x))) # Die Ausgabe der ersten Schicht wird durch die zweite vollverbundene Schicht fc2 geleitet, dann durch die Batch Normalisierung und dann erneut durch die leaky ReLU-Aktivierungsfunktion. Das Ergebnis wird wieder in x gespeichert.
