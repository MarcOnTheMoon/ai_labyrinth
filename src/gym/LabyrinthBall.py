"""
creates the realistic behavior of the ball on the field for a labyrinth OpenAI gym environment.
...
class LabyrinthBall:
    ...
    def __init__(self, geometry):
        ...
        self.__g = 9.81 * 100 #acceleration of gravity in cm/s²
        self.__5_7g = 5.0 / 7.0 * self.__g # constant for ball acceleration
        self.__urr = 0.00118 #Rolling friction coefficient
        self.__velocity = vec(0.0, 0.0, 0.0)    #velocity of the ball
        self.__position = vec(0.0, 0.0, 0.0)    #ballposition
    ...
    def calc_corners(self):
        ...
        """
        #interior walls
        walls_data = self.__geometry.walls.walls_data
        #exterior walls
    ...
    def collision_hole(self):
        ...
        holes_data = self.__geometry.holes.holes_data
        for hole_data in holes_data:
            hole_center = hole_data["pos"]
            if ((self.__position.x - hole_center.x) * (self.__position.x - hole_center.x) + (self.__position.y - hole_center.y) * (self.__position.y - hole_center.y)) < self.__geometry.holes.holes_radius * self.__geometry.holes.holes_radius:
                print("holecollision")
                self.game_state = -1
